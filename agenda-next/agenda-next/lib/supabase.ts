...
type Nota = { id: string; text: string; reminder: string | null }
...
export default function Home() {
  ...
  const [remIdx, setRemIdx] = useState(0)
  const [remDatetime, setRemDatetime] = useState('')
  const [notif, setNotif] = useState('')

  useEffect(() => {
    const saved = localStorage.getItem('agenda_notes')
  ...
  useEffect(() => {
    ...
    const check = setInterval(() => {
      ...
      const now = new Date()
      Object.values(n).flat().forEach((nota: any) => {
        if (!nota.reminder) return
        const t = new Date(nota.reminder)
        if (Math.abs(t.getTime() - now.getTime()) < 30000) {
          showNotif('⏰ ' + nota.text.slice(0,60))
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('📒 Agenda – Promemoria', { body: nota.text })
          }
  ...
  function saveNotes(n: {[k:string]: Nota[]}) {
    setNotes(n)
    localStorage.setItem('agenda_notes', JSON.stringify(n))
  }

  function showNotif(msg: string) { setNotif(msg); setTimeout(()=>setNotif(''), 5000) }
  ...
  function aggiungiNota() {
    ...
    if (!testo) return
    const nuova: Nota = { id: Date.now().toString(), text: testo, reminder: null }
    saveNotes({...notes, [k]: [...lista, nuova]})
    setNuovaNota('')
  }

  function eliminaNota(k: string, i: number) {
    const lista = [...(notes[k] || [])]
    lista.splice(i, 1)
    const ag = {...notes}
    if (lista.length === 0) delete ag[k]
    else ag[k] = lista
    saveNotes(ag)
  }

  function apriPromemoria(k: string, i: number) {
    setRemKey(k); setRemIdx(i)
    const nota = notes[k][i]
  ...
  function salvaPromemoria() {
    if (!remDatetime) return
    const dataOra = new Date(remDatetime)
    if (dataOra <= new Date()) { showNotif('Scegli una data nel futuro'); return }
    const lista = [...(notes[remKey] || [])]
    lista[remIdx] = {...lista[remIdx], reminder: dataOra.toISOString()}
    saveNotes({...notes, [remKey]: lista})
    setModalOpen(false)
  ...
  return (
    <div className="w-full max-w-[430px] mx-auto h-dvh flex flex-col">
      ...
      <div className="bg-[#f5eecc] flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto relative" style={{background:'repeating-linear-gradient(to bottom,#f5eecc 0px,#f5eecc 40px,#c8b870 40px,#c8b870 41px)'}}>
            ...
            {lista.length === 0 ? (
              ...
              <div key={nota.id} className="flex gap-2 py-2 border-b border-[#c8b870] min-h-[41px]">
                ...
                <div className="flex-1">
                  <div className="text-[15px] text-[#1a1a18] leading-snug" style={{fontFamily:'Georgia,serif'}}>{nota.text}</div>
                  {nota.reminder && (
                  ...
                  <div className="flex gap-1.5 mt-1.5">
                    <button className="bg-[#0d1b3e] text-white text-xs px-2.5 py-1 rounded-md" onClick={()=>apriPromemoria(k,i)}>⏰ promemoria</button>
                    <button className="bg-[#7a2020] text-white text-xs px-2.5 py-1 rounded-md" onClick={()=>eliminaNota(k,i)}>elimina</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-[#e8dda0] border-t border-[#c8b870] px-3 py-2.5 flex gap-2 items-end flex-shrink-0">
          <textarea
            ...
            rows={1}
          />
          <button className="bg-[#0d1b3e] text-white rounded-xl px-4 h-[42px] text-sm font-medium flex-shrink-0" onClick={aggiungiNota}>Aggiungi</button>
        </div>
